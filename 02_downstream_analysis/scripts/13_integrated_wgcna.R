rm(list = ls()); gc()
ORIGINAL_DIR <- "."
output <- file.path(ORIGINAL_DIR, "13_WGCNA_All")
...
group<-read.csv('./00_prepared_data/03.All_group.csv')%>% setNames(c('sample', 'type', 'sequencing_batch', 'replicate'))
stopifnot(nrow(group) == 24, !anyDuplicated(group$sample), all(c('Batch1','Batch2') %in% group$sequencing_batch))
...
group2 <- read.csv('./00_prepared_data/03.All_group.csv') %>% 
  setNames(c('ID', 'type', 'sequencing_batch', 'replicate'))
...
png(file.path(output,"04.Module-trait_heatmap.png"),width = 12,height = 10,units = "in",res = 600)
par(mar = c(8, 8, 3, 3)) 
labeledHeatmap(
  Matrix = as.matrix(moduleTraitCor),
  main = paste('Module-trait relationships'),
