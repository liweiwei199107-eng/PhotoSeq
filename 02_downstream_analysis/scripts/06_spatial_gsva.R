rm(list = ls()); gc()
ORIGINAL_DIR <- "."
output <- file.path(ORIGINAL_DIR, "06_GSVA_Spatial")
...
group_levels <- c(
  "Breast_tumor", "Breast_normal_adjacent", "Breast_normal",
  "Lung_metastasis", "Lung_normal_adjacent", "Lung_normal"
)
group$group <- factor(group$group, levels = group_levels)
if (any(is.na(group$group)) || any(table(group$group) != 3)) {
  stop("GSVA-Spatial requires 6 groups with N=3 each.")
}
group <- group[order(group$group, group$id), , drop = FALSE]
# genes <- read_csv("02_Venn_Spatial/02.Venn_DEGs.csv")
...
ann_colors <- list(
  Group = c(
    "Breast normal"          = "#4575B4",   # dark blue
    "Breast normal adjacent" = "#F9804B",   # orange
    "Breast tumor"           = "#69bccE",   # light cyan
    "Lung metastasis"        = "#D73027",   # dark red
    "Lung normal"            = "#91BFDB",   # light blue
    "Lung normal adjacent"   = "#FEE090"    # pale yellow
  )
)
