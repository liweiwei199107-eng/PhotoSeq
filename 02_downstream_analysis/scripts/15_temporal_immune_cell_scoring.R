rm(list = ls()); gc()
ORIGINAL_DIR <- "."
output <- file.path(ORIGINAL_DIR, "15_Immune_Temporal")
...
library(GSVA)
library(pheatmap)
gene_set <- read.table("reference/immune/mmc3_ssGSEA_mouse.txt",
                       header = T,
                       sep ="\t",
                       check.names = FALSE)
...
comparisons <- list(
  c("Breast mid", "Breast early"),
  c("Breast late", "Breast mid"),
  c("Lung metastasis","Breast late")
)
...
for (comp in comparisons) {
  ...
  sample1 <- group$id_normalized[group$Group_plot == group1]
  sample2 <- group$id_normalized[group$Group_plot == group2]
