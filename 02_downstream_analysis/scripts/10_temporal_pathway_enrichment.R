rm(list = ls()); gc()
ORIGINAL_DIR <- "."
output <- file.path(ORIGINAL_DIR, "10_Enrich_Temporal")
...
genes <- read.csv("09_Venn_Temporal/02.Venn_DEGs.csv", stringsAsFactors = FALSE)
...
#         'dephosphorylation'
# )
# data <- df[df$Description %in% BP,]%>%  arrange(pvalue)
...
p1 = ggplot(data,aes(x=Count,y=Description))+
  ...
  scale_x_continuous(limits = c(0, max(data$Count)+1),  
                     expand = c(0, 0))+
  ...
  guides(size = 'none')
p1
ggsave(file.path(output, '01.GO_BP.pdf'), p1, width = 7, height = 6)
ggsave(file.path(output, '01.GO_BP.png'), p1, width = 7, height = 6)
...
p3 = ggplot(data,aes(x=Count,y=Description))+
  ...
  guides(size = 'none')
p3
ggsave(file.path(output, '03.GO_MF.pdf'), p3, width = 7, height = 6)
ggsave(file.path(output, '03.GO_MF.png'), p3, width = 7, height = 6)
...
p5 = ggplot(data,aes(x=Count,y=Description))+
  ...
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 36))+
  scale_x_continuous(limits = c(0, max(data$Count)+1),  
                     expand = c(0, 0))+
  theme_bw()+
  theme(aspect.ratio=2/1,
...
p6 = ggplot(data,aes(x=Count,y=Description))+
  ...
  geom_point(aes(size=Count,color=-log10(p.adjust)))+
  scale_color_gradient(low='#db534c',high="#fbbcba")+
  labs(title="Reactome",x="Count",y="",color=expression(-log[10](adjusted~italic(P)~value)),size="Gene number")+
  scale_x_continuous(limits = c(0, max(data$Count)+1),  
                     expand = c(0, 0))+
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 42))+
  theme_bw()+
  ...
  guides(size = 'none')
p6
ggsave(file.path(output, '05.Reactome.pdf'), p6, width = 7, height = 6)
ggsave(file.path(output, '05.Reactome.png'), p6, width = 7, height = 6)
